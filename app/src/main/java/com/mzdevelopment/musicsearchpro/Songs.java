...
public class Songs extends ActionBarActivity
{
    ...
    private static void applyBlur()
    {
        iv_image.getViewTreeObserver().addOnPreDrawListener(new android.view.ViewTreeObserver.OnPreDrawListener() {
            ...
            {
                Songs.iv_image.getViewTreeObserver().removeOnPreDrawListener(this);
                Songs.iv_image.buildDrawingCache();
                Bitmap bitmap = Songs.iv_image.getDrawingCache();
                if(android.os.Build.VERSION.SDK_INT >= 17)
                {
                    Songs.blur(bitmap, Songs.toplayout, 10F);
                } else
                {
                    BitmapDrawable bitmapdrawable = new BitmapDrawable(bitmap);
                    bitmapdrawable.setAlpha(127);
                    Songs.toplayout.setBackgroundDrawable(bitmapdrawable);
                }
                return true;
            }
    ...
    public static void changeButton()
    {
        runOnUI(new Runnable() {
            ...
            public void run()
            {
                ...
                {
                    Songs.iv_pause.setVisibility(View.GONE);
                    Songs.iv_play.setVisibility(View.VISIBLE);
                    Songs.tv_now_playing.setVisibility(View.GONE);
                    return;
                } else
                {
                    Songs.iv_pause.setVisibility(View.VISIBLE);
                    Songs.iv_play.setVisibility(View.GONE);
                    Songs.tv_now_playing.setVisibility(View.VISIBLE);
                    return;
                }
            }
        ...
        runOnUI(new Runnable() {
            ...
            public void run()
            {
                try
                {
                    ...
                    Songs.imageLoader.DisplayImage(s1, Songs.iv_image);
                    Songs.imageLoader.DisplayImageBitmap(s1, Songs.iv_round, "");
                    Songs.tv_title.setText(s);
                    Songs.tv_artist.setText(s2);
                    Songs.applyBlur();
                    return;
                }
    ...
    public void fillData(String s)
    {
        if(s.equals(""))
            ...
            PlayerConstants.SONGS_LIST_TEMP = db.getAllSongsSearch(s.trim());
        customAdapter = new CustomAdapter(this, R.layout.custom_list, PlayerConstants.SONGS_LIST_TEMP, "songs");
    ...
    public void setDrawer()
    {
        ...
        mDrawerList.setOnItemClickListener(new android.widget.AdapterView.OnItemClickListener() {
            ...
            {
                if(j == 0) {
                    Intent intent = new Intent(Songs.this, PlayList.class);
                    intent.putExtra("pagename", "chart");
                    startActivity(intent);
                    finish();
                } else if(j == 1) {
                    Intent intent1 = new Intent(Songs.this, Search.class);
                    startActivity(intent1);
                } else if(j == 2) {
                    Intent intent2 = new Intent(Songs.this, RecentSongList.class);
                    intent2.putExtra("playlist", "recent");
                    startActivity(intent2);
                    finish();
                } else if(j == 3) {
                    Intent intent3 = new Intent(Songs.this, Songs.class);
                    startActivity(intent3);
                    finish();
                } else if(j == 4) {
                    Intent intent4 = new Intent(Songs.this, Artist.class);
                    startActivity(intent4);
                    finish();
                } else if(j == 5) {
                    Intent intent5 = new Intent(Songs.this, PlayList.class);
